...
public class BinarySearchTreeDeleteRebuildPostProcessor<V> implements BinarySearchTreeDeletePostProcessor<V> {
	...
	private BinarySearchTreeNode<V> createBalancedTreeFrom(List<BinarySearchTreeNode<V>> nodes){
		Integer fromIndex = 0;
		Integer toIndex = nodes.size()-1;
		Integer rootIndex = this.getRootIndexFor(fromIndex, toIndex);
		this.doCreateBalancedTreeFrom(nodes, fromIndex, toIndex);
		return nodes.get(rootIndex);
	}
	
	private int getRootIndexFor(Integer fromIndex, Integer toIndex) {
		return Math.floorDiv(fromIndex + toIndex, 2);
	}
	
	private void doCreateBalancedTreeFrom(List<BinarySearchTreeNode<V>> nodes, Integer fromIndex, Integer toIndex){
	...
	private boolean shouldContinue(Integer fromIndex, Integer toIndex) {
		return fromIndex != toIndex;
	}
	
	private boolean shouldProcessTwoNodes(Integer fromIndex, Integer toIndex) {
		return fromIndex + 1 == toIndex;
	}
	
	private void processTwoNodes(List<BinarySearchTreeNode<V>> nodes, Integer fromIndex, Integer toIndex){
	...
	private void processMoreThanTwoNodes(List<BinarySearchTreeNode<V>> nodes, Integer fromIndex, Integer toIndex){
		Integer rootIndex = this.getRootIndexFor(fromIndex, toIndex);
		Integer leftChildIndex = this.getRootIndexFor(fromIndex, rootIndex-1);
		Integer rightChildIndex = this.getRootIndexFor(rootIndex+1, toIndex);
	...
	private void linkParentWithChildren(List<BinarySearchTreeNode<V>> nodes, Integer rootIndex, Integer leftChildIndex, Integer rightChildIndex){
